...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      _initScroll() {
        if (!this.scroll) {
            console.log('没有scroll时', this.seller);
            this.scroll = new BScroll(this.$refs.seller, {
            click: true
            });
        } else {
          console.log('有scroll时', this.seller);
          this.scroll.refresh();
        }
      },
      _initPics() {
        console.log('this.seller.pics', this.seller.pics);
        if (this.seller.pics) {
          let picWidth = 120;
          let margin = 6;
          let width = (picWidth + margin) * this.seller.pics.length - margin;
          console.log('this.$refs.piclist', this.$refs.piclist);
